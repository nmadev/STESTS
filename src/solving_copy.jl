...
function setEDConstraints(
    ...
    edmodel,
    EDV, EDW, EDU, U,
    EDGPIni, EDSteps, BAWindow,
    strategic, bidmodels,
    db, cb, vrt,
    segment_length, ESSeg, RTDBids, RTCBids, 
    AdjustedEDL, AdjustedEDSolar, AdjustedEDWind,
    all_UCprices_df, all_EDprices_df,
    EDHorizon, d, h, t, ts
)
...
function writeEDtoCSVandUpdateSOC(
    ...
    output_folder::String,
    edmodel,
    EDDBidInput, EDCBidInput,
    EDU, EDV, EDW,
    db, cb,
    all_EDprices_df, storagezone,
    PriceCap, FuelAdjustment,
    EDGSMC,
    EDGMCcost, EDGSMCcost,
    EDVOLL, EDEScost, EDcost,
    EDSteps, ESSeg, d, h, t
)
    ts = ((d - 1) * 24 + h - 1) * EDSteps + t # time step
    EDGMCcost[ts] =
        sum(value.(edmodel[:guc])[:, 1] .* 
        params.GMC * FuelAdjustment
        ) / EDSteps
    EDGSMCcost[ts] =
        sum(
...
function solving(
    ...
    for d in 1:Nday
        ...
        setUCConstraints(
            ucmodel, ucpmodel,
            LInput, HAvailInput, WAvailInput, SAvailInput,
            DADBids, DAdb, DAcb,
            UCHorizon, RM
        )
        ...
        if termination_status(ucmodel) == MOI.OPTIMAL
            GMCcost[d] = sum(value.(ucmodel[:guc])[:, 1:24] .* params.GMC * FuelAdjustment,)
            GSMCcost[d] = sum(value.(ucmodel[:gucs])[:, :, 1:24] .* GSMC[:, :, 1:24] * FuelAdjustment)
            ...
            for h in 1:24
                for t in 1:EDSteps
                    ...
                    if termination_status(edmodel) == MOI.OPTIMAL
                        EDGPIni, EDSOCini, all_EDprices_df = writeEDtoCSVandUpdateSOC(
                            params,
                            output_folder::String,
                            edmodel,
                            EDDBidInput, EDCBidInput,
